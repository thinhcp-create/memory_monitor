...
#             return
#         try:
#             start_addr = int(self.start_input.text(), 16)
#             end_addr = int(self.end_input.text(), 16)
#             if start_addr >= end_addr:
#                 raise ValueError("Địa chỉ bắt đầu phải nhỏ hơn kết thúc.")
#             # if start_addr % 4096 != 0 or end_addr % 4096 != 0:
#             #     raise ValueError("Cả 2 địa chỉ phải là bội số của 4096.")

#             self.buffer = b""
#             self.expected_size = end_addr - start_addr
#             self.start_address = start_addr
#             self.bytes_shown = 0  # reset lại số byte đã hiển thị
#             self.table.setRowCount(0)  # <<< xóa dữ liệu cũ trong bảng
#             cmd = f"READ:{start_addr},{end_addr}"
...
#     #     self.buffer += data  # Thêm dữ liệu vào bộ đệm
#     #     self.fill_table(self.buffer)  # Hiển thị ngay lập tức
#     def handle_data(self, data):
#         try:
...
class SerialApp(QWidget):
    ...
    def read_next_chunk(self):
        ...
        cmd = f"READ:{self.current_address},{chunk_end}\n"
        self.serial.write(cmd.encode())
        print(f"Gửi lệnh: {cmd.strip()}")
